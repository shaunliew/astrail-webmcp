...
- **`move_place` and `set_trip_dates` have not been run against a live trip.** They are implemented and unit-tested; that is the honest state. (`replan_trip` was in this list until 30 Aug, when it was run through the agent after an add and after a remove, and the rewritten title and day summaries were checked in the database. Everything in that list was first run against a backend on `localhost`; the arc, including a real generation, was re-driven against the deployed URL on 2026-09-03. See the README for what each does and does not prove.)
