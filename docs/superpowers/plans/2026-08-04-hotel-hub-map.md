...
- **[T4-REVIEW FOLD — pgTAP execution test]** Add `supabase/tests/019_fenced_hotel_suggestions_replace.sql` mirroring the sibling `supabase/tests/011_fenced_trip_itinerary_replace.sql`: shape checks (function exists, `SECURITY DEFINER`, empty `search_path`, PUBLIC/anon/authenticated denied + service_role granted) + behavioral fence proofs (valid lease writes, stale lease refused with no clobber, cross-trip lease refused, delete-then-insert, jsonb→type casts, NOT-NULL coalesce defaults). The `replace_hotel_suggestions` fence class only fails at RUNTIME (e.g. a `uuid = text` error), so an execution test is the only regression guard — the T4 reviewer verified this live (16/16) but nothing persists into CI without this file.
