...
- **Auth on every endpoint except `/health`** (guardrail #5). The POST route authenticates from the `Authorization` header; the SSE stream route authenticates from a `?token=` query param (with header fallback) because browser `EventSource` cannot send headers. Both validate the same Supabase HS256 JWT.
...
- **Env vars** (per `.claude/docs/ENV.md`): `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `SUPABASE_JWT_SECRET`, `APIFY_TOKEN`, `OPENAI_API_KEY`. Read via `os.environ[...]` (fail fast if absent); never hardcode.
