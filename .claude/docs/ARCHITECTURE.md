...
- **The WebMCP edit endpoints** — `POST /trips/:tripId/places`, `PATCH`/`DELETE /trips/:tripId/places/:tpId`, `PATCH /trips/:tripId`, `POST /trips/:tripId/replan`. All gated by `WEBMCP_EDITS_ENABLED` (default **off**, `main.py`) — note the flag makes the ENDPOINTS 404, it does not hide the tools, which register either way. All 404-not-403 on a foreign trip, all refuse a trip that is unfinished or has a running job, all resequence days so pin numbers stay hole-free.
  **`POST /places` can spend money.** Since `45a918c` it geocodes an unrecognised place name rather than asking the agent for coordinates: first an `ilike` match across `places` gated on the trip's city/country (free), then **Mapbox forward geocoding — a paid call, $5/1,000, no free tier**. A result is accepted only if its country is one of the trip's AND it lands within 500 km of the nearest existing stop; anything else falls back to the old `422` asking for `lat`/`lng`. The created row carries Mapbox's country as its `grounded` receipt **on purpose**: without it the row has NULL city and country, can never match the free gate again, and every later add of the same name re-pays. This is the first edit-route dependency on `MAPBOX_SECRET_TOKEN` — absent, it degrades to the `422` rather than failing.
