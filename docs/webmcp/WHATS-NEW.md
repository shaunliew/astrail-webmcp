...
- **Every tool that writes has now been run against a live trip.** `move_place` and `set_trip_dates` were the last two holding out; both ran on 3 Sept against the deployed URL, each through its own approval card, and both are recorded in the README with the replies they returned. (`replan_trip` left this list on 30 Aug, when it was run through the agent after an add and after a remove, and the rewritten title and day summaries were checked in the database.) The 30 Aug runs were against a backend on `localhost`; everything dated 3 Sept was against the deployment a judge opens.
