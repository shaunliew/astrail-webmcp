...
async def resolve_cached(
    ...
    *,
    ...
    Paid-resolve telemetry lives at the paid-call site (`geocode.hotel_resolver._resolve_one`), NOT here:
    this generic primitive also runs the `resolver()` for a ZERO-cost outcome (a hotel that returns before
    any Mapbox/LLM call), so a log line here would over-count actual billing.
    """
    if client is None:
