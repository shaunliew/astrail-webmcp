...
| 5 | **Frontend** — delete card + **type-to-confirm** dialog + pending banner(+`deleting` no-cancel) + cancel + rewired clear-memory + `backend-types` + readiness-gated | confirm requires typing email/DELETE; failure never shows "deleted"; self-serve hidden pre-flip; `deleting` can't be cancelled in UI |
