/* The two challenge panels: what is new for this hackathon, and how a judge should test it.
 *
 * Lifted out of `app/page.tsx` on 2026-09-02 so it can render INSIDE the story, immediately
 * before the footer. It used to sit after `<StoryStage />`, which renders `<StoryFooter />`
 * itself — so the setup instructions appeared BELOW the footer, past the point a reader treats
 * a page as finished.
 *
 * THE JUDGE CREDENTIALS ARE NOT READ HERE, AND MUST NOT BE ADDED BACK. This markup used to
 * render them from `NEXT_PUBLIC_DEMO_EMAIL` / `NEXT_PUBLIC_DEMO_PASSWORD`. `NEXT_PUBLIC_*` is
 * INLINED INTO THE CLIENT BUNDLE at build time, so those values were readable out of the shipped
 * JavaScript by anyone, whether or not a component printed them — deleting the markup would not
 * have been a fix. The reads had to go, and the deployment sets neither var. It is a working
 * login to an account that spends real Apify and OpenAI credit.
 *
 * Judges get them from Devpost's private testing-instructions field, which only Devpost and the
