...
2. ~~**Duplicate coord-bearing place names in one trip**~~ **FIXED (commit `42f2f7a`):** the shipped code assigns day/sort by place IDENTITY via the shared `feasibility.group_places_by_day` helper (eval-safe — `ItineraryOutput` shape unchanged, eval still `6229.0`), so two same-named places land on their correct days. NOTE: the Task 1 code sample above shows the superseded name-keyed `_day_lookup` approach; the shipped `pipeline/persist.py` (signature `persist_itinerary(client, trip_id, canonical, dates)`, returning the dropped-place count) is authoritative.
