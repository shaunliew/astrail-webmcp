-- Create Saved Reel organize jobs atomically and reject active item overlap.
...
begin
  if p_saved_reel_ids is null
    ...
    raise exception 'Saved Reel organize request is invalid' using errcode = 'P0001';
  end if;
  ...
  if cardinality(v_owned_ids) <> cardinality(p_saved_reel_ids) then
    raise exception 'Saved Reel not found' using errcode = 'P0001';
  end if;
  ...
  if exists (
    ...
    raise exception 'Saved Reel is already being organized' using errcode = 'P0001';
  end if;
