...
| Frontend animation (2nd) | `gsap` (^3.15.0) — **Added 2026-07-30 (Zhi Hao, explicit)** for the card-fan carousel (`components/ui/card-fan-carousel.tsx`, Library panel's reel browser). A pre-built third-party component authored in gsap's imperative timeline (elastic-ease fan choreography + per-slot hover physics); integrated verbatim rather than ported. **Deviation, logged deliberately:** this is the *second* animation lib against the "lesser is better" + "motion is the one animation lib" stance, and against the trays-plan constraint "no new deps beyond motion". Accepted by Zhi Hao 2026-07-30 for behavioral fidelity over a risky gsap→motion port. Do NOT reach for gsap for new work — `motion` remains the default; gsap is scoped to this one imported component |
| Frontend animation (3rd, landing-only) | `lenis` (^1) — **Added 2026-08-02 (Zhi Hao, explicit)** for the `/` scroll-story landing: weighted, eased page scrolling so the One-Stage scroll-mapped camera transitions don't stutter on raw wheel steps (the oryzo-style seamless grammar Zhi Hao asked for). Scoped to the landing (`components/story/`); the app shell must NOT adopt it. `motion` remains the default animation lib elsewhere |
